import { 
  Controller,  Get,  Post,  Delete, Param, Body, UseGuards,  Request, Query,  UnauthorizedException } from '@nestjs/common'; 
...
export class SocialController {
  ...
  @Get('leaderboard')
  getLeaderboard(@Query('limit') limit: number = 100) {
    return this.socialService.getLeaderboard(limit);
  }
  ...
  async getFollowingPublic(@Param('userId') userId: string) {
  ...
  updateCopySettings(
    ...
    @Body() settings: { riskPercent?: number; maxTrades?: number; autoCopy?: boolean }, 
    @Request() req: any
  ) {
  ...
  shareTrade(
    ...
    return this.socialService.shareTrade(req.user.userId, tradeId, message);
  }
}
